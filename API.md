...
| `/chat/sessions/<id>/messages/` | POST | Yes | path **`id`**; body: **`content`**, `mentioned_user_ids` `[ids]` | **SSE** stream of `data: {"text": "..."}` chunks, terminated by `data: [DONE]`. Returns `503` if the Gemini API key is not configured. |
